...
describe('WorkflowRunsTableComponent', () => {
  ...
  describe('getWorkflowStatusClass', () => {
    ...
    const cases: Array<[Partial<WorkflowRunDto>, WorkflowStatusClass]> = [
      ...
      [{ status: 'QUEUED' }, 'text-yellow-500 dark:text-yellow-400'],
      [{ status: 'WAITING' }, 'text-yellow-500 dark:text-yellow-400'],
      [{ status: 'PENDING' }, 'text-yellow-500 dark:text-yellow-400'],
      [{ status: 'REQUESTED' }, 'text-yellow-500 dark:text-yellow-400'],
