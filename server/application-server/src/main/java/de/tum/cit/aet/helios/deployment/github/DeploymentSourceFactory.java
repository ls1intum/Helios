...
public class DeploymentSourceFactory {
    ...
    public DeploymentSource create(Object source) {
        if (source instanceof GHDeployment) {
            return new GHDeploymentSourceAdapter((GHDeployment) source);
        } else if (source instanceof GitHubDeploymentDto) {
            return new GitHubDeploymentSourceAdapter((GitHubDeploymentDto) source);
        } else {
            throw new IllegalArgumentException("Unsupported source type for deployment conversion: " + source.getClass().getName());
        }
    }
}