...
public class GitHubPullRequestConverter
    extends BaseGitServiceEntityConverter<GHPullRequest, PullRequest> {
  ...
  public PullRequest update(@NonNull GHPullRequest source, @NonNull PullRequest pullRequest) {
    ...
    try {
      ...
      log.error(
          "Failed to convert mergeCommitSha field for source {}: {}",
          source.getId(),
          e.getMessage());
    }
    try {
    ...
    try {
      ...
      log.error(
          "Failed to convert mergeable field for source {}: {}", source.getId(), e.getMessage());
    }
    try {
      pullRequest.setMergeableState(source.getMergeableState());
    } catch (IOException e) {
      log.error(
          "Failed to convert mergeableState field for source {}: {}",
          source.getId(),
          e.getMessage());
    }
    try {
    ...
    try {
      ...
      log.error(
          "Failed to convert commits field for source {}: {}", source.getId(), e.getMessage());
    }
    try {
      ...
      log.error(
          "Failed to convert additions field for source {}: {}", source.getId(), e.getMessage());
    }
    try {
      ...
      log.error(
          "Failed to convert deletions field for source {}: {}", source.getId(), e.getMessage());
    }
    try {
      pullRequest.setChangedFiles(source.getChangedFiles());
    } catch (IOException e) {
      log.error(
          "Failed to convert changedFiles field for source {}: {}", source.getId(), e.getMessage());
    }

    pullRequest.setHeadRefName(source.getHead().getRef());
    pullRequest.setHeadSha(source.getHead().getSha());
    pullRequest.setHeadRefRepoNameWithOwner(source.getHead().getRepository().getFullName());
