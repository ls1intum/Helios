...
public class GitHubWorkflowSyncService {
    ...
    public GitHubWorkflowSyncService(
     ...
     *
     * @param repositories the list of GitHub repositories to sync workflow runs
     *                     from
     * @param since        an optional date to filter pull requests by their last
     *                     update
     * @return a list of GitHub workflow runs that were successfully fetched and
     *         processed
     */
    public List<GHWorkflowRun> syncRunsOfAllRepositories(List<GHRepository> repositories,
     ...
     * @param since      an optional date to filter workflow runs by their last
     *                   update
     * @return a list of GitHub workflow runs requests that were successfully fetched and
     *         processed
     */
    public List<GHWorkflowRun> syncRunsOfRepository(GHRepository repository, Optional<OffsetDateTime> since) {
        var iterator = repository.queryWorkflowRuns()
                .list()
                .withPageSize(100)
                .iterator();
            ...
            var keepPullRequests = ghPullRequests.stream()
                    .filter(pullRequest -> {
                        try {
                            ...
                            log.error("Failed to filter workflow run {}: {}", pullRequest.getId(), e.getMessage());
                            return false;
                        }
                    })
                    .toList();
    ...
    public WorkflowRun processRun(GHWorkflowRun ghWorkflowRun) {
        var result = workflowRunRepository.findById(ghWorkflowRun.getId())
                .map(workflowRun -> {
                    try {
                        if (workflowRun.getUpdatedAt() == null || workflowRun.getUpdatedAt()
                                .isBefore(DateUtil.convertToOffsetDateTime(ghWorkflowRun.getUpdatedAt()))) {
                            return workflowRunConverter.update(ghWorkflowRun, workflowRun);
                        }
                        return workflowRun;
                    } catch (IOException e) {
                        log.error("Failed to update worfklow run {}: {}", ghWorkflowRun.getId(), e.getMessage());
                        return null;
                    }
        ...
        try {
            ...
            ghWorkflowRun.getPullRequests().forEach(pullRequest -> {
            ...
            log.error("Failed to process pull requests for workflow run {}: {}", ghWorkflowRun.getId(), e.getMessage());
        }
