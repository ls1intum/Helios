 ...
 * information, such as the limit, remaining requests, used requests, and reset time. The
 * interceptor updates this information every time a GitHub API request is made, ensuring that the
 * application always has the latest rate limit status.
 *
 ...
 *       `latestRateLimitInfo` field.
 *   <li>Logging or displaying the current rate limit status for debugging or informational
 *       purposes.
 *   <li>Implementing custom logic based on the rate limit status, such as delaying requests or
 *       notifying users when the rate limit is close to being reached.
 * </ul>
 */
@Getter
@Setter
